...
class PoseTrajectoryInterpolator:
    def __init__(self, times: np.ndarray, poses: np.ndarray):
        ...
        if len(times) > 1:
            ...
            if not np.all(np.diff(times) > 0):
                ...
                logger.warning("Times are not strictly increasing. Adding small deltas.")
        ...
        if len(times) == 1:
            ...
            pos = self._poses[:,:3]
            self.pos_interp = si.interp1d(self._times, pos, axis=0, bounds_error=False, fill_value="extrapolate")
            ...
            try:
                ...
                self.pos_interp = si.interp1d(times, pos, axis=0, bounds_error=False, fill_value="extrapolate")
    ...
    def trim(self, start_t: float, end_t: float) -> "PoseTrajectoryInterpolator":
        ...
        if not np.any(should_keep):
            ...
            return PoseTrajectoryInterpolator(
                times=np.array([start_t, end_t]),
                poses=np.stack([start_pose, end_pose])
            )
        ...
        return PoseTrajectoryInterpolator(
            times=np.array(times_to_use),
            poses=np.array(poses_to_use)
        )
    ...
    def drive_to_waypoint(self, 
        ...
        return PoseTrajectoryInterpolator(
            times=np.array([curr_time, target_time]),
            poses=np.stack([curr_pose, pose])
        )
