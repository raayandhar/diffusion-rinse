...
class RealCoffeeImageDataset(BaseImageDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
        ...
        # Ensure all three cameras are present in rgb_keys
        required_cameras = ['camera1_rgb', 'camera2_rgb', 'camera3_rgb']
        for cam in required_cameras:
            if cam not in rgb_keys:
                print(f"Warning: {cam} not found in shape_meta.")
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        # reshape all input before fitting
        def flatten(x):
            # Convert zarr array to numpy array first
            x_np = x[:]  # or np.array(x)
            return x_np.reshape(-1, x_np.shape[-1]) if x_np.ndim > 1 else x_np.reshape(-1, 1)

        # action
        normalizer['action'] = SingleFieldLinearNormalizer.create_fit(
            flatten(self.replay_buffer['action']))
        
        # obs
        for key in self.lowdim_keys:
            normalizer[key] = SingleFieldLinearNormalizer.create_fit(
               flatten(self.replay_buffer[key]))
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    print(f"[INFO] Loaded action shape: {action_shape}")
    ...
    for key, shape in lowdim_shapes.items():
        print(f"[INFO] Key '{key}' has shape {shape} — no slicing applied.")

    # Verify camera data presence
    for cam in ['camera1_rgb', 'camera2_rgb', 'camera3_rgb']:
        if cam in rgb_keys:
            print(f"[INFO] Found {cam} data with resolution {out_resolutions[cam]}")
